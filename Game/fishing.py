...
class Fishing: #класс рыбалка
    ...
    def go_fishing(self,i:int):#начало рыалки
        ...
        self.get_fish(i, ran)#сохранение

    def get_fish(self, i:int, ran:int) ->Fish:#сохранение=
        for i in range(len(self.shans)-1):#проходит 100 раз 
            if (self.shans[i][0] <= ran & ran <= [i][1]):#шанс
                return Fish(self.vid[i], self.price[i], random.randint(*self.vid[i]))#соронение пораметров
        return None
